...
-type action(FZB) :: stop | {continue, FZB, fun(() -> action(FZB))}.

-opaque yielder(FZC) :: {yielder, fun(() -> action(FZC))}.

-type step(FZD, FZE) :: {next, FZD, FZE} | done.

-type chunk(FZF, FZG) :: {another_by,
        list(FZF),
        FZG,
        FZF,
        fun(() -> action(FZF))} |
...
-type sized_chunk(FZH) :: {another, list(FZH), fun(() -> action(FZH))} |
    {last, list(FZH)} |
    no_more.
...
-spec unfold_loop(FZP, fun((FZP) -> step(FZQ, FZP))) -> fun(() -> action(FZQ)).
...
-spec unfold(FZK, fun((FZK) -> step(FZL, FZK))) -> yielder(FZL).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> FZU)) -> yielder(FZU).
repeatedly(F) ->
...
-spec from_list(list(FZY)) -> yielder(FZY).
from_list(List) ->
...
-spec transform(yielder(GAI), GAK, fun((GAK, GAI) -> step(GAL, GAK))) -> yielder(GAL).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(GAS)), fun((GAU, GAS) -> GAU), GAU) -> GAU.
...
-spec fold(yielder(GAP), GAR, fun((GAR, GAP) -> GAR)) -> GAR.
...
-spec to_list(yielder(GAX)) -> list(GAX).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(GBA)) -> step(GBA, yielder(GBA)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(GBI)), integer()) -> fun(() -> action(GBI)).
...
-spec take(yielder(GBF), integer()) -> yielder(GBF).
...
-spec drop_loop(fun(() -> action(GBO)), integer()) -> action(GBO).
...
-spec map(yielder(GBR), fun((GBR) -> GBT)) -> yielder(GBT).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(GBZ), yielder(GCB), fun((GBZ, GCB) -> GCD)) -> yielder(GCD).
...
-spec append_loop(fun(() -> action(GCP)), fun(() -> action(GCP))) -> action(GCP).
...
-spec append(yielder(GCL), yielder(GCL)) -> yielder(GCL).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(GCX)))) -> action(GCX).
...
-spec flatten(yielder(yielder(GCT))) -> yielder(GCT).
flatten(Yielder) ->
...
-spec concat(list(yielder(GDB))) -> yielder(GDB).
concat(Yielders) ->
...
-spec flat_map(yielder(GDF), fun((GDF) -> yielder(GDH))) -> yielder(GDH).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(GDN)), fun((GDN) -> boolean())) -> action(GDN).
...
-spec filter(yielder(GDK), fun((GDK) -> boolean())) -> yielder(GDK).
...
-spec filter_map(yielder(GDQ), fun((GDQ) -> {ok, GDS} | {error, any()})) -> yielder(GDS).
...
-spec cycle(yielder(GEE)) -> yielder(GEE).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(GEM)), fun((GEM) -> boolean())) -> {ok, GEM} |
    {error, nil}.
...
-spec find(yielder(GEI), fun((GEI) -> boolean())) -> {ok, GEI} | {error, nil}.
...
-spec find_map(yielder(GEQ), fun((GEQ) -> {ok, GES} | {error, any()})) -> {ok,
        GES} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(GFJ)), integer()) -> fun(() -> action({GFJ,
    integer()})).
...
-spec index(yielder(GFG)) -> yielder({GFG, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(GFM, fun((GFM) -> GFM)) -> yielder(GFM).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(GFR)), fun((GFR) -> boolean())) -> fun(() -> action(GFR)).
...
-spec take_while(yielder(GFO), fun((GFO) -> boolean())) -> yielder(GFO).
...
-spec drop_while_loop(fun(() -> action(GFX)), fun((GFX) -> boolean())) -> action(GFX).
...
-spec drop_while(yielder(GFU), fun((GFU) -> boolean())) -> yielder(GFU).
...
-spec scan_loop(fun(() -> action(GGE)), fun((GGG, GGE) -> GGG), GGG) -> fun(() -> action(GGG)).
...
-spec scan(yielder(GGA), GGC, fun((GGC, GGA) -> GGC)) -> yielder(GGC).
...
-spec zip_loop(fun(() -> action(GGN)), fun(() -> action(GGP))) -> fun(() -> action({GGN,
    GGP})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(GGI), yielder(GGK)) -> yielder({GGI, GGK}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(GHC)), fun((GHC) -> GHE), GHE, list(GHC)) -> chunk(GHC, GHE).
...
-spec chunk_loop(fun(() -> action(GGX)), fun((GGX) -> GGZ), GGZ, GGX) -> action(list(GGX)).
...
-spec chunk(yielder(GGS), fun((GGS) -> any())) -> yielder(list(GGS)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(GHQ)), integer(), list(GHQ)) -> sized_chunk(GHQ).
...
-spec sized_chunk_loop(fun(() -> action(GHM)), integer()) -> fun(() -> action(list(GHM))).
...
-spec sized_chunk(yielder(GHI), integer()) -> yielder(list(GHI)).
...
-spec intersperse_loop(fun(() -> action(GHX)), GHX) -> action(GHX).
...
-spec intersperse(yielder(GHU), GHU) -> yielder(GHU).
...
-spec any_loop(fun(() -> action(GIC)), fun((GIC) -> boolean())) -> boolean().
...
-spec any(yielder(GIA), fun((GIA) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(GIG)), fun((GIG) -> boolean())) -> boolean().
...
-spec all(yielder(GIE), fun((GIE) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(GIW) -> fun((gleam@option:option(list(GIW))) -> list(GIW)).
...
-spec group_updater(fun((GIO) -> GIP)) -> fun((gleam@dict:dict(GIP, list(GIO)), GIO) -> gleam@dict:dict(GIP, list(GIO))).
group_updater(F) ->
...
-spec group(yielder(GII), fun((GII) -> GIK)) -> gleam@dict:dict(GIK, list(GII)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(GJA), fun((GJA, GJA) -> GJA)) -> {ok, GJA} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(GJE)) -> {ok, GJE} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(GJM) -> yielder(GJM).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(GJS)), fun(() -> action(GJS))) -> action(GJS).
...
-spec interleave(yielder(GJO), yielder(GJO)) -> yielder(GJO).
...
-spec fold_until_loop(
    fun(() -> action(GKA)),
    fun((GKC, GKA) -> gleam@list:continue_or_stop(GKC)),
    GKC
) -> GKC.
...
-spec fold_until(
    yielder(GJW),
    GJY,
    fun((GJY, GJW) -> gleam@list:continue_or_stop(GJY))
) -> GJY.
...
-spec try_fold(yielder(GKE), GKG, fun((GKG, GKE) -> {ok, GKG} | {error, GKH})) -> {ok,
        GKG} |
    {error, GKH}.
...
-spec first(yielder(GKU)) -> {ok, GKU} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(GKY), integer()) -> {ok, GKY} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(GLG), fun((GLG) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(GLJ, fun(() -> yielder(GLJ))) -> yielder(GLJ).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(GLM), GLM) -> yielder(GLM).
