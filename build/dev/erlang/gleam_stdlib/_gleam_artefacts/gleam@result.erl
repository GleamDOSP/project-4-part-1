...
-spec map({ok, CQC} | {error, CQD}, fun((CQC) -> CQG)) -> {ok, CQG} |
    {error, CQD}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CQJ} | {error, CQK}, fun((CQK) -> CQN)) -> {ok, CQJ} |
    {error, CQN}.
...
-spec flatten({ok, {ok, CQQ} | {error, CQR}} | {error, CQR}) -> {ok, CQQ} |
    {error, CQR}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CQY} | {error, CQZ}, fun((CQY) -> {ok, CRC} | {error, CQZ})) -> {ok,
        CRC} |
    {error, CQZ}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CRH} | {error, CRI}, fun((CRH) -> {ok, CRL} | {error, CRI})) -> {ok,
        CRL} |
    {error, CRI}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CRQ} | {error, any()}, CRQ) -> CRQ.
...
-spec lazy_unwrap({ok, CRU} | {error, any()}, fun(() -> CRU)) -> CRU.
...
-spec unwrap_error({ok, any()} | {error, CRZ}, CRZ) -> CRZ.
...
-spec unwrap_both({ok, CSC} | {error, CSC}) -> CSC.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, CSF} | {error, CSG}, {ok, CSF} | {error, CSG}) -> {ok, CSF} |
    {error, CSG}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CSN} | {error, CSO}, fun(() -> {ok, CSN} | {error, CSO})) -> {ok,
        CSN} |
    {error, CSO}.
...
-spec all(list({ok, CSV} | {error, CSW})) -> {ok, list(CSV)} | {error, CSW}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 353).
-spec partition_loop(list({ok, CTK} | {error, CTL}), list(CTK), list(CTL)) -> {list(CTK),
    list(CTL)}.
...
-spec partition(list({ok, CTD} | {error, CTE})) -> {list(CTD), list(CTE)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CTT}, CTW) -> {ok, CTW} | {error, CTT}.
...
-spec replace_error({ok, CTZ} | {error, any()}, CUD) -> {ok, CTZ} | {error, CUD}.
...
-spec values(list({ok, CUG} | {error, any()})) -> list(CUG).
values(Results) ->
