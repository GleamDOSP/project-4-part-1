...
-type dict(LQ, LR) :: any() | {gleam_phantom, LQ, LR}.
...
-spec to_list(dict(MA, MB)) -> list({MA, MB}).
to_list(Dict) ->
    maps:to_list(Dict).
...
-spec get(dict(ND, NE), ND) -> {ok, NE} | {error, nil}.
get(From, Get) ->
...
-spec has_key(dict(MR, any()), MR) -> boolean().
has_key(Dict, Key) ->
...
-spec insert(dict(NJ, NK), NJ, NK) -> dict(NJ, NK).
insert(Dict, Key, Value) ->
    maps:put(Key, Value, Dict).

-file("src/gleam/dict.gleam", 92).
-spec from_list_loop(list({MK, ML}), dict(MK, ML)) -> dict(MK, ML).
...
-spec from_list(list({MF, MG})) -> dict(MF, MG).
from_list(List) ->
    maps:from_list(List).

-file("src/gleam/dict.gleam", 223).
-spec reverse_and_concat(list(OT), list(OT)) -> list(OT).
...
-spec do_keys_loop(list({OO, any()}), list(OO)) -> list(OO).
...
-spec keys(dict(OJ, any())) -> list(OJ).
keys(Dict) ->
    maps:keys(Dict).

-file("src/gleam/dict.gleam", 249).
-spec do_values_loop(list({any(), PD}), list(PD)) -> list(PD).
...
-spec do_take_loop(dict(QH, QI), list(QH), dict(QH, QI)) -> dict(QH, QI).
...
-spec delete(dict(RL, RM), RL) -> dict(RL, RM).
delete(Dict, Key) ->
...
-spec drop(dict(RX, RY), list(RX)) -> dict(RX, RY).
...
-spec upsert(dict(SE, SF), SE, fun((gleam@option:option(SF)) -> SF)) -> dict(SE, SF).
upsert(Dict, Key, Fun) ->
...
-spec fold_loop(list({SQ, SR}), ST, fun((ST, SQ, SR) -> ST)) -> ST.
...
-spec map_values(dict(NV, NW), fun((NV, NW) -> NZ)) -> dict(NV, NZ).
map_values(Dict, Fun) ->
    maps:map(Fun, Dict).
...
-spec each(dict(SU, SV), fun((SU, SV) -> any())) -> nil.
each(Dict, Fun) ->
    fold(
        Dict,
        nil,
...
-spec combine(dict(SZ, TA), dict(SZ, TA), fun((TA, TA) -> TA)) -> dict(SZ, TA).
