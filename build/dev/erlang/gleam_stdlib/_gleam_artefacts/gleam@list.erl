...
-type continue_or_stop(YW) :: {continue, YW} | {stop, YW}.
...
-spec count_loop(list(AAD), fun((AAD) -> boolean()), integer()) -> integer().
...
-spec count(list(AAB), fun((AAB) -> boolean())) -> integer().
...
-spec reverse(list(AAF)) -> list(AAF).
reverse(List) ->
    lists:reverse(List).
...
-spec group_loop(list(ABF), fun((ABF) -> ABH), gleam@dict:dict(ABH, list(ABF))) -> gleam@dict:dict(ABH, list(ABF)).
...
-spec group(list(AAZ), fun((AAZ) -> ABB)) -> gleam@dict:dict(ABB, list(AAZ)).
group(List, Key) ->
    group_loop(List, Key, maps:new()).

-file("src/gleam/list.gleam", 339).
-spec filter_loop(list(ABR), fun((ABR) -> boolean()), list(ABR)) -> list(ABR).
...
-spec filter(list(ABO), fun((ABO) -> boolean())) -> list(ABO).
...
-spec map_loop(list(ACO), fun((ACO) -> ACQ), list(ACQ)) -> list(ACQ).
...
-spec map(list(ACK), fun((ACK) -> ACM)) -> list(ACM).
map(List, Fun) ->
    map_loop(List, Fun, []).

-file("src/gleam/list.gleam", 429).
-spec map2_loop(list(ACZ), list(ADB), fun((ACZ, ADB) -> ADD), list(ADD)) -> list(ADD).
...
-spec map_fold_loop(list(ADL), fun((ADN, ADL) -> {ADN, ADO}), ADN, list(ADO)) -> {ADN,
    list(ADO)}.
...
-spec index_map(list(ADR), fun((ADR, integer()) -> ADT)) -> list(ADT).
index_map(List, Fun) ->
    index_map_loop(List, Fun, 0, []).

-file("src/gleam/list.gleam", 548).
-spec try_map_loop(list(AEJ), fun((AEJ) -> {ok, AEL} | {error, AEM}), list(AEL)) -> {ok,
        list(AEL)} |
    {error, AEM}.
...
-spec try_map(list(AEA), fun((AEA) -> {ok, AEC} | {error, AED})) -> {ok,
        list(AEC)} |
    {error, AED}.
try_map(List, Fun) ->
...
-spec drop(list(AET), integer()) -> list(AET).
drop(List, N) ->
    case N =< 0 of
        true ->
            List;
...
-spec take_loop(list(AEZ), integer(), list(AEZ)) -> list(AEZ).
...
-spec take(list(AEW), integer()) -> list(AEW).
take(List, N) ->
    take_loop(List, N, []).
...
-spec wrap(AFF) -> list(AFF).
wrap(Item) ->
    [Item].

-file("src/gleam/list.gleam", 679).
-spec append_loop(list(AFL), list(AFL)) -> list(AFL).
...
-spec append(list(AFH), list(AFH)) -> list(AFH).
append(First, Second) ->
...
-spec prepend(list(AFP), AFP) -> list(AFP).
prepend(List, Item) ->
    [Item | List].

-file("src/gleam/list.gleam", 720).
-spec flatten_loop(list(list(AFW)), list(AFW)) -> list(AFW).
...
-spec flat_map(list(AGB), fun((AGB) -> list(AGD))) -> list(AGD).
flat_map(List, Fun) ->
...
-spec fold(list(AGG), AGI, fun((AGI, AGG) -> AGI)) -> AGI.
...
-spec fold_right(list(AGJ), AGL, fun((AGL, AGJ) -> AGL)) -> AGL.
...
-spec index_fold(list(AGM), AGO, fun((AGO, AGM, integer()) -> AGO)) -> AGO.
...
-spec try_fold(list(AGS), AGU, fun((AGU, AGS) -> {ok, AGU} | {error, AGV})) -> {ok,
        AGU} |
    {error, AGV}.
...
-spec fold_until(list(AHA), AHC, fun((AHC, AHA) -> continue_or_stop(AHC))) -> AHC.
...
-spec find(list(AHE), fun((AHE) -> boolean())) -> {ok, AHE} | {error, nil}.
...
-spec find_map(list(AHI), fun((AHI) -> {ok, AHK} | {error, any()})) -> {ok, AHK} |
    {error, nil}.
find_map(List, Fun) ->
    case List of
        [] ->
...
-spec all(list(AHQ), fun((AHQ) -> boolean())) -> boolean().
all(List, Predicate) ->
    case List of
        [] ->
            true;
...
-spec any(list(AHS), fun((AHS) -> boolean())) -> boolean().
any(List, Predicate) ->
    case List of
        [] ->
            false;
...
-spec zip_loop(list(AHZ), list(AIB), list({AHZ, AIB})) -> list({AHZ, AIB}).
...
-spec strict_zip_loop(list(AIM), list(AIO), list({AIM, AIO})) -> {ok,
        list({AIM, AIO})} |
    {error, nil}.
...
-spec strict_zip(list(AIF), list(AIH)) -> {ok, list({AIF, AIH})} | {error, nil}.
strict_zip(List, Other) ->
    strict_zip_loop(List, Other, []).

-file("src/gleam/list.gleam", 1138).
-spec unzip_loop(list({AIZ, AJA}), list(AIZ), list(AJA)) -> {list(AIZ),
    list(AJA)}.
...
-spec unzip(list({AIU, AIV})) -> {list(AIU), list(AIV)}.
unzip(Input) ->
    unzip_loop(Input, [], []).

-file("src/gleam/list.gleam", 1173).
-spec intersperse_loop(list(AJJ), AJJ, list(AJJ)) -> list(AJJ).
...
-spec intersperse(list(AJG), AJG) -> list(AJG).
...
-spec unique_loop(list(AJQ), gleam@dict:dict(AJQ, nil), list(AJQ)) -> list(AJQ).
...
-spec sort(list(AJW), fun((AJW, AJW) -> gleam@order:order())) -> list(AJW).
sort(List, Compare) ->
    case List of
        [] ->
            [];
...
-spec repeat_loop(ALL, integer(), list(ALL)) -> list(ALL).
...
-spec repeat(ALJ, integer()) -> list(ALJ).
repeat(A, Times) ->
    repeat_loop(A, Times, []).

-file("src/gleam/list.gleam", 1556).
-spec split_loop(list(ALS), integer(), list(ALS)) -> {list(ALS), list(ALS)}.
...
-spec split(list(ALO), integer()) -> {list(ALO), list(ALO)}.
split(List, Index) ->
    split_loop(List, Index, []).

-file("src/gleam/list.gleam", 1592).
-spec split_while_loop(list(AMB), fun((AMB) -> boolean()), list(AMB)) -> {list(AMB),
    list(AMB)}.
...
-spec split_while(list(ALX), fun((ALX) -> boolean())) -> {list(ALX), list(ALX)}.
...
-spec key_find(list({AMG, AMH}), AMG) -> {ok, AMH} | {error, nil}.
...
-spec key_filter(list({AML, AMM}), AML) -> list(AMM).
...
-spec key_pop_loop(list({AMV, AMW}), AMV, list({AMV, AMW})) -> {ok,
        {AMW, list({AMV, AMW})}} |
    {error, nil}.
...
-spec key_set_loop(list({ANG, ANH}), ANG, ANH, list({ANG, ANH})) -> list({ANG,
    ANH}).
...
-spec each(list(ANL), fun((ANL) -> any())) -> nil.
each(List, F) ->
    case List of
        [] ->
            nil;
...
-spec try_each(list(ANO), fun((ANO) -> {ok, any()} | {error, ANR})) -> {ok, nil} |
    {error, ANR}.
try_each(List, Fun) ->
    case List of
        [] ->
...
-spec partition_loop(list(BHH), fun((BHH) -> boolean()), list(BHH), list(BHH)) -> {list(BHH),
    list(BHH)}.
...
-spec window_loop(list(list(APD)), list(APD), integer()) -> list(list(APD)).
...
-spec window(list(AOZ), integer()) -> list(list(AOZ)).
window(List, N) ->
    case N =< 0 of
        true ->
            [];
...
-spec window_by_2(list(APJ)) -> list({APJ, APJ}).
window_by_2(List) ->
...
-spec drop_while(list(APM), fun((APM) -> boolean())) -> list(APM).
...
-spec take_while_loop(list(APS), fun((APS) -> boolean()), list(APS)) -> list(APS).
...
-spec chunk_loop(list(AQB), fun((AQB) -> AQD), AQD, list(AQB), list(list(AQB))) -> list(list(AQB)).
...
-spec chunk(list(APW), fun((APW) -> any())) -> list(list(APW)).
chunk(List, F) ->
    case List of
        [] ->
            [];
...
-spec reduce(list(AQU), fun((AQU, AQU) -> AQU)) -> {ok, AQU} | {error, nil}.
reduce(List, Fun) ->
    case List of
        [] ->
            {error, nil};
...
-spec scan_loop(list(ARC), ARE, list(ARE), fun((ARE, ARC) -> ARE)) -> list(ARE).
...
-spec scan(list(AQY), ARA, fun((ARA, AQY) -> ARA)) -> list(ARA).
...
-spec last(list(ARH)) -> {ok, ARH} | {error, nil}.
last(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec combinations(list(ARL), integer()) -> list(list(ARL)).
...
-spec combination_pairs_loop(list(ARS), list({ARS, ARS})) -> list({ARS, ARS}).
...
-spec take_firsts(list(list(ASM)), list(ASM), list(list(ASM))) -> {list(ASM),
    list(list(ASM))}.
...
-spec transpose_loop(list(list(ASF)), list(list(ASF))) -> list(list(ASF)).
...
-spec transpose(list(list(ASA))) -> list(list(ASA)).
...
-spec interleave(list(list(ARW))) -> list(ARW).
interleave(List) ->
    _pipe = List,
...
-spec shuffle_pair_unwrap_loop(list({float(), ASY}), list(ASY)) -> list(ASY).
...
-spec do_shuffle_by_pair_indexes(list({float(), ATC})) -> list({float(), ATC}).
...
-spec max_loop(list(ATM), fun((ATM, ATM) -> gleam@order:order()), ATM) -> ATM.
...
-spec max(list(ATF), fun((ATF, ATF) -> gleam@order:order())) -> {ok, ATF} |
    {error, nil}.
max(List, Compare) ->
    case List of
        [] ->
...
-spec build_reservoir_loop(
    list(AUB),
    integer(),
    gleam@dict:dict(integer(), AUB)
) -> {gleam@dict:dict(integer(), AUB), list(AUB)}.
...
-spec build_reservoir(list(ATW), integer()) -> {gleam@dict:dict(integer(), ATW),
    list(ATW)}.
...
-spec sample(list(ATN), integer()) -> list(ATN).
sample(List, N) ->
...
-spec permutation_zip(list(AOJ), list(AOJ), list(list(AOJ))) -> list(list(AOJ)).
...
-spec permutations(list(AOF)) -> list(list(AOF)).
permutations(List) ->
    case List of
        [] ->
            [[]];
