...
-spec map_first({CPL, CPM}, fun((CPL) -> CPN)) -> {CPN, CPM}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CPO, CPP}, fun((CPP) -> CPQ)) -> {CPO, CPQ}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
