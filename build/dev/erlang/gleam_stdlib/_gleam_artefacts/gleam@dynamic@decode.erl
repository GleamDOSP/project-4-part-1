...
-spec run(gleam@dynamic:dynamic_(), decoder(BYS)) -> {ok, BYS} |
...
-spec map(decoder(CCQ), fun((CCQ) -> CCS)) -> decoder(CCS).
...
-spec then(decoder(CDC), fun((CDC) -> decoder(CDE))) -> decoder(CDE).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CDH), list(decoder(CDH))) -> decoder(CDH).
...
-spec recursive(fun(() -> decoder(CDX))) -> decoder(CDX).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(CCM)) -> decoder(gleam@option:option(CCM)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CAN} | {error, CAN})
) -> {CAN, list(decode_error())}.
...
-spec collapse_errors(decoder(CCZ), binary()) -> decoder(CCZ).
...
-spec failure(CDR, binary()) -> decoder(CDR).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CDT} | {error, CDT})
) -> decoder(CDT).
...
-spec fold_dict(
    {gleam@dict:dict(CBY, CBZ), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {CBY, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {CBZ, list(decode_error())})
) -> {gleam@dict:dict(CBY, CBZ), list(decode_error())}.
...
-spec dict(decoder(CBR), decoder(CBT)) -> decoder(gleam@dict:dict(CBR, CBT)).
dict(Key, Value) ->
...
-spec list(decoder(CBF)) -> decoder(list(CBF)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BZO, list(decode_error())}, list(any())) -> {BZO,
    list(decode_error())}.
...
-spec index(
    ...
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BZC)) -> {BZF, list(decode_error())})
) -> {BZF, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BYN), fun((BYN) -> decoder(BYP))) -> decoder(BYP).
...
-spec at(list(any()), decoder(BYZ)) -> decoder(BYZ).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BZX), fun((BZX) -> decoder(BZZ))) -> decoder(BZZ).
...
-spec optional_field(any(), CAD, decoder(CAD), fun((CAD) -> decoder(CAF))) -> decoder(CAF).
...
-spec optionally_at(list(any()), CAK, decoder(CAK)) -> decoder(CAK).
