...
-opaque set(CZL) :: {set, gleam@dict:dict(CZL, list(nil))}.
...
-spec contains(set(CZW), CZW) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CZY), CZY) -> set(CZY).
delete(Set, Member) ->
...
-spec fold(set(DAH), DAJ, fun((DAJ, DAH) -> DAJ)) -> DAJ.
...
-spec filter(set(DAK), fun((DAK) -> boolean())) -> set(DAK).
...
-spec drop(set(DAR), list(DAR)) -> set(DAR).
drop(Set, Disallowed) ->
...
-spec take(set(DAV), list(DAV)) -> set(DAV).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 287).
-spec order(set(DBD), set(DBD)) -> {set(DBD), set(DBD)}.
order(First, Second) ->
...
-spec intersection(set(DBI), set(DBI)) -> set(DBI).
...
-spec difference(set(DBM), set(DBM)) -> set(DBM).
...
-spec is_subset(set(DBQ), set(DBQ)) -> boolean().
...
-spec is_disjoint(set(DBT), set(DBT)) -> boolean().
...
-spec each(set(DCA), fun((DCA) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CZT), CZT) -> set(CZT).
insert(Set, Member) ->
...
-spec from_list(list(DAE)) -> set(DAE).
from_list(Members) ->
...
-spec map(set(DAN), fun((DAN) -> DAP)) -> set(DAP).
map(Set, Fun) ->
...
-spec symmetric_difference(set(DBW), set(DBW)) -> set(DBW).
