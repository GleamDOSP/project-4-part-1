...
-spec reverse_and_prepend(list(HF), list(HF)) -> list(HF).
...
-spec all_loop(list(option(GW)), list(GW)) -> option(list(GW)).
all_loop(List, Acc) ->
    case List of
        [] ->
...
-spec all(list(option(GR))) -> option(list(GR)).
all(List) ->
    all_loop(List, []).
...
-spec to_result(option(HN), HQ) -> {ok, HN} | {error, HQ}.
to_result(Option, E) ->
    case Option of
        {some, A} ->
            {ok, A};
...
-spec from_result({ok, HT} | {error, any()}) -> option(HT).
from_result(Result) ->
    case Result of
        {ok, A} ->
            {some, A};
...
-spec lazy_unwrap(option(IA), fun(() -> IA)) -> IA.
...
-spec map(option(IC), fun((IC) -> IE)) -> option(IE).
map(Option, Fun) ->
    case Option of
        {some, X} ->
            {some, Fun(X)};
...
-spec flatten(option(option(IG))) -> option(IG).
flatten(Option) ->
    case Option of
        {some, X} ->
            X;
...
-spec then(option(IK), fun((IK) -> option(IM))) -> option(IM).
then(Option, Fun) ->
    case Option of
        {some, X} ->
            Fun(X);
...
-spec 'or'(option(IP), option(IP)) -> option(IP).
'or'(First, Second) ->
    case First of
        {some, _} ->
            First;
...
-spec lazy_or(option(IT), fun(() -> option(IT))) -> option(IT).
...
-spec values_loop(list(option(JB)), list(JB)) -> list(JB).
...
-spec values(list(option(IX))) -> list(IX).
values(Options) ->
    values_loop(Options, []).
