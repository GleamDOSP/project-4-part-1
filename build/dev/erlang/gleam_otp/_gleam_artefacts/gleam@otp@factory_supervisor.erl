...
-opaque supervisor(EXA, EXB) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(EXA, EXB))}.

-type message(EXC, EXD) :: any() | {gleam_phantom, EXC, EXD}.

-opaque builder(EXE, EXF) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((EXE) -> {ok, gleam@otp@actor:started(EXF)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(EXE, EXF)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(EXG, EXH) :: {local,
        gleam@erlang@process:name(message(EXG, EXH))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(EXI) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, EXI}.
...
-type erlang_child_spec_property(EXJ, EXK) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((EXJ) -> {ok, gleam@otp@actor:started(EXK)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(EXL, EXM))) -> supervisor(EXL, EXM).
get_by_name(Name) ->
...
-spec named(builder(EYC, EYD), gleam@erlang@process:name(message(EYC, EYD))) -> builder(EYC, EYD).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(EYL, EYM), integer(), integer()) -> builder(EYL, EYM).
...
-spec timeout(builder(EYR, EYS), integer()) -> builder(EYR, EYS).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(EYX, EYY), gleam@otp@supervision:restart()) -> builder(EYX, EYY).
...
-spec start(builder(EZD, EZE)) -> {ok,
        gleam@otp@actor:started(supervisor(EZD, EZE))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(FAC, FAD)) -> gleam@otp@supervision:child_specification(supervisor(FAC, FAD)).
supervised(Builder) ->
...
-spec start_child(supervisor(FAJ, FAK), FAJ) -> {ok,
        gleam@otp@actor:started(FAK)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((FBH) -> {ok, gleam@otp@actor:started(FBI)} |
        {error, gleam@otp@actor:start_error()}),
    FBH
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), FBI, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((EXS) -> {ok, gleam@otp@actor:started(EXT)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(EXS, EXT).
...
-spec supervisor_child(
    fun((EXX) -> {ok, gleam@otp@actor:started(EXY)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(EXX, EXY).
