...
-type message(EBE) :: {message, EBE} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(EBF, EBG) :: {continue,
        EBF,
        gleam@option:option(gleam@erlang@process:selector(EBG))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(EBH, EBI) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        EBH,
        gleam@erlang@process:selector(message(EBI)),
        gleam@otp@system:debug_state(),
        fun((EBH, EBI) -> next(EBH, EBI))}.

-type started(EBJ) :: {started, gleam@erlang@process:pid_(), EBJ}.

-opaque initialised(EBK, EBL, EBM) :: {initialised,
        EBK,
        gleam@option:option(gleam@erlang@process:selector(EBL)),
        EBM}.

-opaque builder(EBN, EBO, EBP) :: {builder,
        fun((gleam@erlang@process:subject(EBO)) -> {ok,
                initialised(EBN, EBO, EBP)} |
            {error, binary()}),
        integer(),
        fun((EBN, EBO) -> next(EBN, EBO)),
        gleam@option:option(gleam@erlang@process:name(EBO))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(EBQ) :: {ack, {ok, EBQ} | {error, binary()}} |
...
-spec continue(EBV) -> next(EBV, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(ECH, ECI), gleam@erlang@process:selector(ECI)) -> next(ECH, ECI).
...
-spec initialised(ECO) -> initialised(ECO, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(ECT, any(), ECV),
    gleam@erlang@process:selector(ECZ)
) -> initialised(ECT, ECZ, ECV).
...
-spec returning(initialised(EDE, EDF, any()), EDK) -> initialised(EDE, EDF, EDK).
...
-spec new(EDO) -> builder(EDO, EDP, gleam@erlang@process:subject(EDP)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(EDU)) -> {ok, initialised(EDW, EDU, EDX)} |
        {error, binary()})
) -> builder(EDW, EDU, EDX).
...
-spec on_message(builder(EEG, EEH, EEI), fun((EEG, EEH) -> next(EEG, EEH))) -> builder(EEG, EEH, EEI).
...
-spec named(builder(EER, EES, EET), gleam@erlang@process:name(EES)) -> builder(EER, EES, EET).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(EFG))) -> gleam@erlang@process:selector(message(EFG)).
...
-spec receive_message(self(any(), EFC)) -> message(EFC).
...
-spec initialise_actor(
    builder(any(), any(), EFZ),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, EFZ} | {error, binary()})
...
-spec start(builder(any(), any(), EGM)) -> {ok, started(EGM)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EGT), EGT) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EGV),
    integer(),
    fun((gleam@erlang@process:subject(EGX)) -> EGV)
) -> EGX.
