...
-type child_specification(EUM) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(EUM)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(EUN)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EUN).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(EUS)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EUS).
supervisor(Start) ->
...
-spec significant(child_specification(EUX), boolean()) -> child_specification(EUX).
...
-spec timeout(child_specification(EVA), integer()) -> child_specification(EVA).
timeout(Child, Ms) ->
...
-spec restart(child_specification(EVD), restart()) -> child_specification(EVD).
...
-spec map_data(child_specification(EVG), fun((EVG) -> EVI)) -> child_specification(EVI).
