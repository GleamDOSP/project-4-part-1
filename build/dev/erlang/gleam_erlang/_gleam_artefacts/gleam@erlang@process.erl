...
-opaque subject(DQH) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DQH)}.

-type name(DQI) :: any() | {gleam_phantom, DQI}.

-type do_not_leak() :: any().

-type selector(DQJ) :: any() | {gleam_phantom, DQJ}.
...
-spec 'receive'(subject(DRH), integer()) -> {ok, DRH} | {error, nil}.
...
-spec receive_forever(subject(DRP)) -> DRP.
...
-spec selector_receive(selector(DRT), integer()) -> {ok, DRT} | {error, nil}.
...
-spec selector_receive_forever(selector(DRX)) -> DRX.
...
-spec map_selector(selector(DRZ), fun((DRZ) -> DSB)) -> selector(DSB).
map_selector(A, B) ->
...
-spec merge_selector(selector(DSD), selector(DSD)) -> selector(DSD).
merge_selector(A, B) ->
...
-spec select_map(selector(DSO), subject(DSQ), fun((DSQ) -> DSO)) -> selector(DSO).
...
-spec select(selector(DSK), subject(DSK)) -> selector(DSK).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DSY)
) -> selector(DSY).
...
-spec select_other(selector(DTC), fun((gleam@dynamic:dynamic_()) -> DTC)) -> selector(DTC).
...
-spec deselect(selector(DST), subject(any())) -> selector(DST).
...
-spec select_specific_monitor(selector(DTO), monitor(), fun((down()) -> DTO)) -> selector(DTO).
...
-spec select_monitors(selector(DTR), fun((down()) -> DTR)) -> selector(DTR).
...
-spec select_trapped_exits(selector(DSH), fun((exit_message()) -> DSH)) -> selector(DSH).
...
-spec deselect_specific_monitor(selector(DTU), monitor()) -> selector(DTU).
...
-spec send_after(subject(DUQ), integer(), DUQ) -> timer().
...
-spec perform_call(
    subject(DTX),
    fun((subject(DTZ)) -> DTX),
    fun((selector(DTZ)) -> {ok, DTZ} | {error, nil})
) -> DTZ.
...
-spec call(subject(DUE), integer(), fun((subject(DUG)) -> DUE)) -> DUG.
...
-spec call_forever(subject(DUI), fun((subject(DUK)) -> DUI)) -> DUK.
