...
-spec be_ok({ok, HHU} | {error, any()}) -> HHU.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, HHZ}) -> HHZ.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(HIC)) -> HIC.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
